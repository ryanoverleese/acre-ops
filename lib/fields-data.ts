...
/**
 * Fetch and process fields data. When season is provided, only fetches
 * field_seasons for that year (and filters probe_assignments to match),
 * which significantly reduces data and speeds up the page.
 */
export async function getFieldsData(season?: number): Promise<FieldsDataResult> {
  try {
    // Build optional season filter for field_seasons
    const seasonFilters: BaserowFilter[] | undefined = season
      ? [{ field: 'season', type: 'equal', value: String(season) }]
      : undefined;

    // Fetch core data — field_seasons filtered by season when provided
    ...
    // Filter probe_assignments to only those linked to loaded field_seasons
    ...
    // Get unique seasons from defaults (current ± 2 years)
    ...
    // Include any seasons from loaded field_seasons
    ...
    // Build probe_assignments lookup by field_season_id
