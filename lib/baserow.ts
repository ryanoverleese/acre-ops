...
export async function ensureSelectOption(
  ...
  try {
    ...
    // Field definition changes require JWT auth (database tokens return 401)
    const jwt = await getBaserowJwt();
    if (!jwt) {
      console.error(`ensureSelectOption: cannot modify field - set BASEROW_EMAIL and BASEROW_PASSWORD env vars`);
      return;
    }
