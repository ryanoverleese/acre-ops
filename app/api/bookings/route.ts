...
import { getCachedRows, type FieldSeason, type Field, type Contact, type Operation } from '@/lib/baserow';
...
export async function GET() {
  try {
    ...
    const bookings: { operationName: string; fields2025: number; fields2026: number; status: string }[] = [];
    ...
    return NextResponse.json([], { status: 200 }); // Return empty, don't error
  }
}
